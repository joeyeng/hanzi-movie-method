  ...
  if (url.pathname === '/hanzi_data.db') {
    event.respondWith(
      fetch(request).catch(() => {
        ...
        return new Response('Database should be loaded from IndexedDB', { status: 503 });
      })
    );
    return;
  }
        ...
        .catch((error) => {
          ...
          return new Response(
            JSON.stringify({ 
              error: 'Offline', 
              offline: true,
              message: 'This feature requires an internet connection or uses the offline database.'
            }),
            { 
              status: 503, 
