...
export function GroupReview({ groupWords, learningData, onExit, onDataChange, groupId, initialFilter = 'reviewed', autoStart = false }: GroupReviewProps) {
    ...
    const buildReviewQueue = useCallback(() => {
        ...
        switch (reviewFilter) {
            case 'reviewed':
                filtered = wordsWithState.filter(w => w.reviewed && !w.learned);
                break;
            case 'unlearned':
    ...
    // Generate pinyin choices (toneless) - fetches from entire database
    const generatePinyinChoices = useCallback(async () => {
        if (!currentItem) return;

        const correctPinyin = normalizePinyin(currentItem.pinyin);
        const charLength = currentItem.word.length;
                    ...
                    <div className="space-y-3 mb-6">
                        <label className="flex items-center gap-3 p-3 bg-slate-700 rounded-lg cursor-pointer hover:bg-slate-600 transition-colors">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="text-sm text-slate-400">{filterCounts.reviewed} words marked for review that haven't been learned yet.</div>
                            </div>
                        </label>
